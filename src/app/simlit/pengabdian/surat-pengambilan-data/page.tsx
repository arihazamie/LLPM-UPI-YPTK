...
export default function SuratTugasPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const list: Pengabdian[] = json?.data || [];
        const pengabdian = list.find((p) => p.id === id);
        if (!pengabdian) throw new Error("Data tidak ditemukan.");

        const dosenList = pengabdian.dosenPengabdian.map((d) => ({
          namaDosen: d.namaDosen ?? "-",
          NIDN: d.NIDN ?? "-",
          prodiFakultas: d.programStudiDosenPengabdian ?? "-",
          role: d.roleDosenPengabdian,
          noHp: d.noHp ?? "-",
        }));
        ...
        const suratData: SuratData = {
          judulPenelitian: pengabdian.judulPengabdian ?? "-",
  ...
  const nomorSuratLengkap = useMemo(() => {
    ...
    return `${nomorUrut}/LPPM.UPI-YPTK/SPD.PKM/HS.V/${bulan}/${tahun}`;
  }, [nomorUrut]);
        ...
        <main
          ...
          <section className="text-sm text-justify mt-6">
            ...
            <p>
              ...
              Pengabdian Kepada Masyarakat kepada dosen kami berikut ini:
            </p>
          </section>
          ...
          <section className="text-sm text-justify mb-4">
            ...
            <p className="mt-2">
              ...
              kegiatan pengabdian di instansi yang Bapak/Ibu pimpin. Ketua dapat
