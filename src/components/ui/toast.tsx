  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        destructive: "border-red-200 bg-red-500 text-white shadow-md",
        success: "border-green-200 bg-green-500 text-white shadow-md",
        warning: "border-amber-200 bg-amber-500 text-white shadow-md",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, duration = 4000, variant = "default", ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "absolute bottom-0 left-0 h-1 bg-black/10 w-full",
        className
      )}
      {...props}>
      <div
...
const Toast = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof ToastPrimitives.Root> &
    VariantProps<typeof toastVariants> & { showProgress?: boolean }
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-white/70 opacity-70 transition-opacity hover:text-white hover:opacity-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-white/20 group-hover:opacity-100",
      className
    )}
    toast-close=""
    {...props}>
